...
void main() {
	...
	findContours(dst_morph, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		if (length > 200 && length < 1000) {
			...
			putText(src, to_string(int(length)), centroid, FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 3);
		}
